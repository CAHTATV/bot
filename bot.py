...
MATCHES_TO_CHECK = 7
GOAL_THRESHOLD = 0.65      
MIN_ODDS = 1.30            # Сигналы от КФ 1.30
    ...
    while True:
        ...
        for m in queue_copy:
            ...
            if force_send:
                ...
                # Безопасная сборка текста без многострочных F-строк
                msg_lines = [
                    "⚽️ СИГНАЛ НА 1 ТАЙМ",
                    league_line,
                    "",
                    ...
                    f"💰 Коэффициент: {odds_str}",
                    "",
                    "Ставка: Тотал 1Т Больше 0.5"
                ]
                text = "\n".join(msg_lines)
    ...
    while True:
        try:
            ...
            for match in data.get("response", []):
                ...
                # НАДЁЖНАЯ СБОРКА ТЕКСТА: Полностью исключает ошибку незакрытых многострочных кавычек
                live_msg_lines = [
                    "🚨 LIVE СИГНАЛ",
                    league_line,
                    "",
        ...
        if now.hour == 23 and now.minute == 59:
            ...
            if today not in report_sent:
                ...
                report_lines = [
                    ...
                    f"Всего сигналов: {len(today_matches)}",
                ...
                if not matches_1t:
                    report_lines.append("  *Сигналов не было*")
                for m in matches_1t:
