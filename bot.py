print("BOT VERSION: MERGED + SSL FIX + 15 MIN 2T DELAY")
...
# ================== КЛЮЧИ ==================
TOKEN = "8402411542:AAHlu7Ob-EEln_0h2Z0-vAWj7GJ_Ft7H_1k"
...
# ================== API С ЗАЩИТОЙ SSL HANDSHAKE ==================
def safe_request(url, params):
    # Создаем стабильную сессию для обхода строгих правил Cloudflare
...
# ================== LIVE ЛАЙВ МОНИТОРИНГ 1ГО ТАЙМА ==================
    ...
    while True:
        try:
            ...
            for match in data.get("response", []):
                ...
                # ИЗМЕНЕНИЕ: Замораживаем сигнал строго на 15 минут (900 секунд) для идеального тайминга под свисток 2Т
                print(f"⏳ Матч {translate_team(home_raw)} в перерыве подходит по условиям. Замораживаем сигнал на 15 минут, чтобы выдать строго перед стартом 2Т...")
                time.sleep(900)
...
# ================== ИНТЕРАКТИВНЫЙ ПОТОК ВХОДЯЩИХ КОМАНД С АНТИ-КОНФЛИКТОМ ==================
    ...
    while True:
        try:
            ...
            for u in updates:
                ...
                if text == "/start":
                    send("👋 Привет! Я твой футбольный бот.\n\n"
            ...
            print("🚨 Конфликт копий! Включаю анти-спам защиту, уступаю поток для сброса старого процесса...")
            time.sleep(12) 
        except Exception as e:
...
threading.Thread(target=live_first_half_monitor, daemon=True).start()  
threading.Thread(target=live_second_half_monitor, daemon=True).start() # Поток задержки 2Т теперь спит 15 минут
