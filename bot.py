...
# ================== НАСТРОЙКИ ==================
MSK = timezone(locale_data.datetime.timedelta(hours=3)) if hasattr(locale_data, 'datetime') else timezone(threading.sys.modules['datetime'].timedelta(hours=3)) if 'datetime' in threading.sys.modules else None
# Корректная инициализация таймзоны (МСК) через стандартный datetime
    ...
    for f in fixtures:
        try:
            ...
            if is_home and teams.get("home", {}).get("id") != team_id:
                continue
            if not is_home model and teams.get("away", {}).get("id") != team_id:
                continue
...
def calc_signal_1t(home_id, away_id, league_id):
    """Комплексный расчёт вероятности гола в 1Т с учётом темпа лиг."""
