print("BOT VERSION: MERGED + ANTI-CONFLICT PROTECTION + COMPACT UI")
...
from telegram.error import RetryAfter, Conflict # Добавили отслеживание конфликта сессий
...
def telegram_commands_loop():
    ...
    while True:
        try:
            ...
            # УМНАЯ ЗАЩИТА: Если старый бот ломится, мы делаем большую паузу.
            # Наш бот засыпает, старый зависший бот шлет тонну пустых запросов, 
            # и алгоритмы Telegram отправляют старый процесс во временный бан за спам-запросы, полностью его очищая!
            print("🚨 Конфликт копий! Включаю анти-спам защиту, уступаю поток для сброса старого процесса...")
            time.sleep(12) # Засыпаем на 12 секунд
        except Exception as e:
