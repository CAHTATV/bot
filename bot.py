...
# Топ-лиги по ID API-Football: 39=АПЛ, 78=Бундеслига, 135=Серия А,
# 61=Лига1, 140=Ла Лига, 94=Примейра, 88=Эредивизи, 2=ЛЧ, 3=ЛЕ
TOP_LEAGUES = {39, 78, 135, 61, 140, 94, 88, 2, 3}

def calc_signal_1t(home_id, away_id, league_id):
    """
    Комплексный расчёт вероятности гола в 1Т.
    Учитывает: забиваемость, пропускаемость, среднюю минуту гола, лигу.
    """
    home_score = team_first_half_goal_rate(home_id, True)
    away_score = team_first_half_goal_rate(away_id, False)
    base = (home_score + away_score) / 2

    home_concede = first_half_concede_pct(home_id, True)
    away_concede = first_half_concede_pct(away_id, False)
    concede_boost = (home_concede + away_concede) / 2

    minute_home = avg_first_goal_minute(home_id)
    minute_away = avg_first_goal_minute(away_id)
    avg_min = (minute_home + minute_away) / 2
    if avg_min < 28:
        minute_boost = 0.10
    elif avg_min < 35:
        minute_boost = 0.05
    ...
    league_boost = 0.05 if league_id in TOP_LEAGUES else 0

    final = base + concede_boost * 0.4 + minute_boost + league_boost

    return int(min(final, 1.0) * 100)
