...
def calc_signal_1t(home_id, away_id):
    ...
    signal = (home_home + away_away) / 2
    return int(signal * 100)
    ...
    for f in fixtures:
        ...
        if is_home and f["teams"]["home"]["id"] != team_id:
            continue
        if not is_home and f["teams"]["away"]["id"] != team_id:
            continue
