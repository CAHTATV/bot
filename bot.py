...
# ================== НАСТРОЙКИ ==================
# Корректная и чистая инициализация таймзоны (МСК)
...
ODDS_CHECK_INTERVAL = 60   # МОНИТОРИНГ КФ КАЖДУЮ МИНУТУ (Улучшено)
...
CACHE_TTL = 36000        # использовать кэш если свежее 10 часов
CACHE_MAX_AGE = 172800   # удалять записи старше 48 часов

# ================== КЭШ МАТЧЕЙ КОМАНДЫ ==================
def cleanup_cache():
    """Удаляет из stats_cache записи старше 48 часов."""
    now = time.time()
...
def get_avg_first_goal_minute(team_id, is_home):
    """ПРОКАЧАННАЯ АНАЛИТИКА: Рассчитывает среднюю минуту первого гола в матче в 1Т."""
    fixtures = get_team_last_matches(team_id).get("response", [])
    minutes = []
    checked = 0
...
def calc_signal_1t(home_id, away_id, league_id):
    """Комплексный расчёт вероятности гола в 1Т с учётом темпа лиг и маркеров времени."""
    ...
    # Проверка темпа лиг из внешнего locale_data
    ...
    # Оценка скорости раскрытия матча (новые маркеры)
    ...
    if avg_first_goal_time <= 23:
        time_boost = 0.10     # Бонус за быстрый гол на старте встречи
    elif avg_first_goal_time >= 35:
        time_boost = -0.08    # Штраф за сухую "раскачку"
    else:
        time_boost = 0
...
def odds_monitor_loop():
    while True:
        ...
        for m in queue_copy:
            ...
            # ЗАЩИТА: Микро-пауза 1.5 сек между запросами, чтобы не ловить лимиты API
            time.sleep(1.5)
