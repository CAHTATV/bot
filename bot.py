        ...
        for m in queue_copy:
            ...
            if odds is None and time_until_match <= 180 and time_until_match > -60:
                ...
                if time_until_match <= 0:
                    with queue_lock:
                        match_queue[:] = [x for x in match_queue if x["match_id"] != match_id]
                    print(f"⏰ Матч начался без нужного КФ, убран: {m['home_ru']} — {m['away_ru']}")
                continue
...
def send(text, specific_chat_id=None):
    # Если указан конкретный chat_id (для команд), шлем только туда
...
# ================== ИНТЕРАКТИВНЫЙ ПОТОК ВХОДЯЩИХ КОМАНД (NEW) ==================
def telegram_commands_loop():
    offset = 0
    print("🤖 Поток прослушивания команд запущен успешно...")
    while True:
        try:
            # Запрашиваем новые сообщения у серверов Telegram
            ...
            for u in updates:
                ...
                # Безопасность: отвечаем только на твои команды по твоему CHAT_ID
                if user_id != CHAT_ID:
                    continue
                ...
                if text == "/start":
                    ...
                    with queue_lock:
                        q = list(match_queue)
                    if not q:
                        send("📋 *Очередь мониторинга пуста.*\nНовые матчи добавятся при следующем автоматическом сканировании.", specific_chat_id=CHAT_ID)
                    else:
                        lines = ["📋 *МАТЧИ В ОЧЕРЕДИ НА МОНИТОРИНГ КФ:*"]
...
threading.Thread(target=telegram_commands_loop, daemon=True).start() # Поток прослушивания команд
