...
def analyze_match(m):
    ...
    # --- Форма последних 5 игр (очень важно)
    def last5_form(games, team_id, home=True):
        cnt = 0
        for g in games[:5]:
            if home and g["teams"]["home"]["id"] == team_id:
                if g["goals"]["home"] > 0:
                    cnt += 1
            if not home and g["teams"]["away"]["id"] == team_id:
                if g["goals"]["away"] > 0:
                    cnt += 1
        return cnt / 5
    ...
    # --- Вероятность BTTS (правильная логика)
