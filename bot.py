...
ODDS_CHECK_INTERVAL = 180  # мониторинг КФ каждые 3 минуты
...
def get_first_half_over_odds(fixture_id):
    """
    Запрашивает коэффициент на Тотал 1Т Больше 0.5 у API-Football.
    Bookmaker 8 = Bet365, bet 23 = First Half Goals Over/Under.
...
# ================== ЗАГРУЗКА МАТЧЕЙ ДНЯ (только очередь, без отправки) ==================
def load_matches():
    global match_queue
    print("LOAD MATCHES...")
    ...
    for f in fixtures:
        try:
            ...
            if match_id in existing_ids:
                continue  # уже в очереди на мониторинг
            ...
            # Матч прошёл — добавляем в очередь для мониторинга КФ
            with queue_lock:
                match_queue.append({
                    "match_id": match_id,
                    "home": home_raw,
...
def odds_monitor_loop():
    """
    Каждые 3 минуты активно проверяет КФ по всем матчам в очереди.
    Как только КФ >= MIN_ODDS — шлёт сигнал и убирает матч из очереди.
    Sleep в конце — чтобы сразу начать работу без начального ожидания.
    """
    while True:
        ...
        for m in queue_copy:
            ...
            # Убираем матчи которые уже начались
            ...
            # Убираем из очереди мониторинга
            with queue_lock:
                match_queue[:] = [x for x in match_queue if x["match_id"] != match_id]

            # Добавляем в отслеживание результатов
            with tracked_lock:
        ...
        time.sleep(ODDS_CHECK_INTERVAL)  # ждём 3 минуты перед следующей проверкой
...
def get_second_half_over_odds(fixture_id):
    """
    Запрашивает live КФ на Тотал 2Т Больше 0.5.
    Bookmaker 8 = Bet365, bet 57 = Second Half Goals Over/Under.
    Возвращает float или None если нет данных.
    """
    try:
        data = safe_request(
    ...
    while True:
        try:
            ...
            for match in data.get("response", []):
                ...
                # Проверяем live КФ на Тотал 2Т > 0.5
                ...
                if odds is not None:
                    odds_line = f"💰 Коэффициент: {odds:.2f}"
                else:
                    odds_line = "💰 Коэффициент: уточните у букмекера"
